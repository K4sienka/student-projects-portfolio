...
oczek<-rep(srednie_opady,length(test_y))
#rzeczywiste wartości
rzeczyw<-test_y

#RMSE
rmse<-sqrt(sum((oczek-rzeczyw)^2)/length(rzeczyw))
cat("Błąd średniokwadratowy wynosi:", rmse)

# Zadanie 3
#musze stworzyc za pomocą lm aby zadzialal predict (nie dziala on z pojedynczymi wartoscami jak srednie_opady)
model<- lm(trening_y ~ 1)
#predict() na zbiorze testowym
przewidywane_opady<-predict(model,testowy)
#rmse
rmse_bazowy<-sqrt(mean((test_y - przewidywane_opady)^2))
cat("Błąd średniokwadratowy (RMSE) dla modelu bazowego z użyciem predict() wynosi:", rmse_bazowy)
